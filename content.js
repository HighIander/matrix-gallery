(() => {
  ...
  function findImageMessages() {
    const containers = Array.from(document.querySelectorAll('[data-event-id], .mx_EventTile, li, [role="listitem"]'));
    const result = [];
  ...
  function findMainImage(element) {
    const images = Array.from(element.querySelectorAll("img")).filter(img => {
      ...
      if (!src) return false;
      if (img.closest("#mg-panel") || img.closest("#mg-toggle") || img.closest(".mg-lightbox")) return false;
  ...
  function installLightboxHandler() {
    const maybeOpen = event => {
      ...
      const message = img.closest("[data-event-id], .mx_EventTile, li, [role='listitem']");
